...
def prefix(a_list):
    ...
    for i in range(0, len(a_list)):  # Προσπέλαση λίστας των προθεματικών αθροισμάτων
        ...
        if i == 0:  
            ...
            prefix_sums.append(a_list[i] + prefix_sums[i - 1])  # Προσθέτουμε το προηγούμενο στοιχείο της λίστας των προθεματικών αθροισμάτων μέ το στοιχείο που βρίσκεται στην θέση i του αρχικού μας πίνακα
...
def MaxSubSlow(a_list):
    ...
    for i in range(0, len(a_list)):
        ...
        for j in range(i, len(a_list)):
            ...
            if largest_sum < sum:    # Αν το άθροισμα που υπολόγισε είναι μεγαλύτερο του μέγιστου
    ...
    return largest_sum, start, end # Επιστρέφει μία λίστα με το μέγιστο άθροισμα, την θέση που ξεκινάει και την θέση που τελειώνει
...
def MaxSubFaster(a_list):
    ...
    for i in range(0, len(a_list)):
        ...
        for j in range(i, len(a_list)):
            ...
            if i == 0:  
                sum = a_list[j]  # Το άθροισμα ισούται με το στοιχείο του πίνακα στην θέση 0
            else:  
                sum = a_list[j] - a_list[i - 1] # Το άθροισμα ισούται με το το στοιχείο που βρίσκεται στην θέση j μείον το στοιχείο που βρίσκεται στην θέση i-1
...
def kadane(a_list):
    ...
    for i in range(0, len(a_list)):
        ...
        if sum < 0:  # Αν το τωρινό άθροισμα είναι μικρότερο του μηδενός
            sum = 0  
            pointer = i + 1  # θέτουμε την επόμενη θέση από αυτή που βρισκόμαστε ώς αρχική για το επομενο άθροισμα
    ...
    for f in functions:
        ...
        for n in num:
            ...
            if f.__name__ == "MaxSubFaster":
                results = f(prefix_sums)  # Αποτελέσματα του δεύτερου αλγορίθμου
                # Καταγραφή της ώρας που παίρνει ο κώδικας να υλοποιηθεί
                duration = timeit.default_timer() - start_time
            else:
                results = f(sequence)  # Αποτελέσματα του τρίτου αλγορίθμου
                # Καταγραφή της ώρας που παίρνει ο κώδικας να υλοποιηθεί
                duration = timeit.default_timer() - start_time
            print(
